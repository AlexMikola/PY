...
from app_1_mains import ClearScene as CS
from app_1_mains import Settings_scen as SC
from app_1_mains import Border_Scene as BS
...
class MyProperties(PropertyGroup):
    ...
    # tmp_dir = r"путь к временному каталогу"
...
class PAN_PT_one(DAD_MT_panel, Panel):
    ...
    def draw(self, context):
        ...
        row2.scale_y = 1.3
        row2.operator('func.func_op', text='Apply render settings', icon='MODIFIER').action = 'RENDSET'
        row2.operator('func.func_op', text='Scene for border', icon='DUPLICATE').action = 'BORDERSCENE'
        row3 = layout.row(align=True)
        row3.scale_y = 1.3
        row3.operator('func.func_op', text='Show all pr buildings', icon='HOME').action = 'BUILDINGS'
        row3.operator('func.func_op', text='Hide project', icon='HIDE_ON').action = 'HIDE_ON'
        row3.operator('func.func_op', text='Show project', icon='HIDE_OFF').action = 'HIDE_OFF'
...
class FUNC_OT_my_func(Operator):
    ...
    action: EnumProperty(
        items=(
            ...
            ('HIDE_ON', 'HIDE_ON', 'Hide_on'),
            ('HIDE_OFF', 'HIDE_OFF', 'Hide_off')
        )
    )
    ...
    def execute(self, context):
        ...
        if self.action == 'LUCK':
            ...
            self.LF.show_filter_viewport()  # вызываем функцию
            ...
            self.LF = LF()                          # инициализируем класс
            self.LF.view_align_by_all_buildings()   # вызываем функцию

        elif self.action == 'HIDE_OFF':
            self.LF = LF()                          # инициализируем класс
            self.LF.vwport_hide()                   # вызываем функцию

        elif self.action == 'HIDE_ON':
            self.LF = LF()                          # инициализируем класс
            self.LF.vwport_show()                   # вызываем функцию
